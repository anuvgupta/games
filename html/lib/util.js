...
var util = {
    ...
    correct_link: (link, block) => {
        var link_t = '';
        if (link.substring(0, 8) == 'flash://') {
            link = link.substring(8).trim();
            if (link != '') {
                //link_t = `${link}`;
                link_t = 'play';
                link = `play/${link}?t=flash`;
                block.child('preview/a').attribute('target', '_blank');
                block.child('info/link/a').attribute('target', '_blank');
            }
        } else if (link.substring(0, 6) == 'web://') {
        ...
        } else if (link.substring(0, 5) == 'dl://') {
            link = link.substring(5).trim();
            if (link != '') {
                link_t = `${link.split('/')[2]}`;
                link = `javascript:app.main.download('${link}', '${link_t}');`;
                block.child('preview/a').attribute('target', '_blank');
                block.child('info/link/a').attribute('target', '_blank');
            }
        } else link_t = link.replace('https://', '').replace('http://', '');
        return [link, link_t];
    }
};